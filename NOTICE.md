...
| https://github.com/EvoLinkAI/awesome-gpt-image-2-API-and-Prompts (X originals linked per-record) | community | @AllaAisling (via EvoLinkAI); @Ciri_ai (via EvoLinkAI); @Diplomeme (via EvoLinkAI); @Gdgtify (via EvoLinkAI); @KimAkiyama81 (via EvoLinkA... | 20 | CC0-1.0 catalog; X originals attributed per record |
...
- fal / PixVerse / EvoLink / Toolcentral / Apiyi / jiling / Kanaries / Ofox / iMini / FindSkill rows may be GPT Image 2–era or 2.5 launch-day;
  structure is compatible with 2.5; see per-record `notes` where present.
- X/Twitter originals are linked in `attribution.url` for `gi25-x-*` records; preview images are cached from the EvoLink mirror.
